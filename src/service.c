                    ...
                    struct sscg_evp_pkey      **_svc_key)
{
    ...
    if (options->subject_alt_names) {
        for (i = 0; options->subject_alt_names[i]; i++) {
            svc_certinfo->subject_alt_names =
                talloc_realloc(svc_certinfo, svc_certinfo->subject_alt_names,
                               char *, i + 2);
            CHECK_MEM(svc_certinfo->subject_alt_names);

            svc_certinfo->subject_alt_names[i] =
                talloc_strdup(svc_certinfo->subject_alt_names,
    ...
    ret = sscg_sign_x509_csr(tmp_ctx, csr, serial, options->lifetime,
                             ca_cert, ca_key, options->hash_fn, &cert);
    CHECK_OK(ret);
