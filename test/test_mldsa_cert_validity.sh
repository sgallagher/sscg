...
# ARG_OPTIONAL_SINGLE([mldsa-nist-level],[],[ML-DSA NIST security level],[2])
# ARG_HELP([Test for validity of created ML-DSA certificates])
# ARGBASH_GO()
...
print_help()
{
	printf '%s\n' "Test for validity of created ML-DSA certificates"
	printf 'Usage: %s [--hash-alg <arg>] [--cipher-alg <arg>] [--mldsa-nist-level <arg>] [-h|--help]\n' "$0"
	printf '\t%s\n' "--hash-alg: Certificate hashing algorithm (default: 'sha256')"
	printf '\t%s\n' "--cipher-alg: Key password cipher algorithm (default: 'aes-256-cbc')"
	printf '\t%s\n' "--mldsa-nist-level: ML-DSA NIST security level (default: '2')"
	printf '\t%s\n' "-h, --help: Prints help"
}
...
parse_commandline()
{
	...
	do
		...
		case "$_key" in
			...
			# See the comment of option '--hash-alg=' to see what's going on here - principle is the same.
			--mldsa-nist-level=*)
				_arg_mldsa_nist_level="${_key##--mldsa-nist-level=}"
				;;
...
TMPDIR=$(mktemp --directory --tmpdir sscg_mldsa_XXXXXX)
...
# Test with hash-alg if provided (it should be ignored for ML-DSA)
if [ -n "${_arg_hash_alg}" ]; then
	echo "Testing that ML-DSA ignores explicit hash-alg..."
	"${MESON_BUILD_ROOT}"/sscg --hash-alg "$_arg_hash_alg" \
	                         --cipher-alg "$_arg_cipher_alg" \
							 --key-type mldsa \
							 --mldsa-nist-level "$_arg_mldsa_nist_level" \
							 --cert-key-password mypassword \
							 --dhparams-file "dhparams.pem"
else
	# No hash-alg specified, run normally
	"${MESON_BUILD_ROOT}"/sscg --cipher-alg "$_arg_cipher_alg" \
							 --key-type mldsa \
							 --mldsa-nist-level "$_arg_mldsa_nist_level" \
							 --cert-key-password mypassword \
							 --dhparams-file "dhparams.pem"
...
# Verify the key type is ML-DSA
key_type=$(openssl pkey -text -noout -in service-key.pem -passin pass:mypassword | head -n1)
echo "$key_type" | grep -i "ML-DSA"

# Verify the NIST level matches what we requested
case "$_arg_mldsa_nist_level" in
	2)
		echo "$key_type" | grep "ML-DSA-44"
		;;
	3)
		echo "$key_type" | grep "ML-DSA-65"
		;;
	5)
		echo "$key_type" | grep "ML-DSA-87"
		;;
	*)
		echo "Unknown NIST level: $_arg_mldsa_nist_level"
		exit 1
		;;
esac
