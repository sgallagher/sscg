...
# ARG_OPTIONAL_SINGLE([remote],[r],[The git remote for the upstream repository.],[origin])
...
print_help()
{
	...
	printf 'Usage: %s [-r|--remote <arg>] [-c|--commitish <arg>] [--(no-)debug] [--(no-)dry-run] [-h|--help] <version>\n' "$0"
	printf '\t%s\n' "<version>: The version of SSCG to release"
	printf '\t%s\n' "-r, --remote: The git remote for the upstream repository. (default: 'origin')"
...
if [ "$_arg_dry_run" != "on" ]; then
	...
	git push ${_arg_remote} main
	git push ${_arg_remote} tag ${tagname}
